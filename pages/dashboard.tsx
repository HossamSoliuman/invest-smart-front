  ...
  async function fetchUser() {
    try {
      ...
      if (token) {
        ...
        setUser({
          ...userData,
          account_id: userData["account_id "].trim(),
        })
      } else {
        ...
        <div className="flex">
          ...
          <div className="flex-1 lg:px-6 max-w-full">
            <div className="text-2xl font-bold text-gray-800 dark:text-gray-100">
              {t("welcome", { name: user?.name || "فلان" })}
            </div>
            <div className=" text-gray-500 dark:text-gray-400 mt-1">
              <span className="font-medium">{t("accountId")}</span>{" "}
              {user?.account_id}
            </div>

            <section className="flex max-md:flex-col gap-2 mt-12">
              <BalanceCard
